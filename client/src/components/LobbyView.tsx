...
export function LobbyView({ gameState, playerName, onStart }: Props) {
  ...
  return (
	  <main className="flex min-h-screen flex-col items-center justify-center bg-black/40 text-white p-4">
		...
		<div className="mb-8 w-full max-w-xs bg-zinc-900/80 p-4 rounded-lg border border-zinc-700 shadow-md flex flex-col gap-2">
		  <div className="flex items-center justify-between">
			<span className="font-medium text-sm text-zinc-200">Extension Loupe 🔍</span>
			{isHost ? (
				<label className={`relative inline-flex items-center ${canUseLoupe ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'}`}>
				  <input
					  type="checkbox"
				...
				<span className={`text-[10px] font-black px-2 py-1 rounded uppercase tracking-wider ${gameState.isLoupeModeEnabled ? 'bg-amber-500 text-black' : 'bg-zinc-700 text-zinc-400'}`}>
                {gameState.isLoupeModeEnabled ? 'Activé' : 'Désactivé'}
              </span>
			)}
		  </div>
		  {!canUseLoupe && isHost && <p className="text-[10px] text-zinc-500 italic text-center mt-1">Nécessite 5 joueurs minimum.</p>}
		</div>
