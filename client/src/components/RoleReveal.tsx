...
export function RoleReveal({ role, revealed, isReady, isConfirming, onReveal }: RoleRevealProps) {
  ...
  return (
	  <div className="relative w-full h-full flex flex-col items-center justify-center">
		...
		{revealed && (
			<div className={`absolute w-72 h-72 sm:w-96 sm:h-96 rounded-full blur-[80px] transition-opacity duration-1000 ${theme.glow}`} />
		)}

		<div className="relative z-10 flex flex-col items-center">
		  {/* TEXTE CINÉMATIQUE (TU ES...) */}
		  <div className={`text-center mb-4 sm:mb-6 landscape:mb-2 transition-all duration-700 ${revealed ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
			<p className="text-zinc-500 font-bold tracking-[0.4em] uppercase text-[10px] sm:text-xs mb-1">Tu es...</p>
		  ...
		  >
			<div className="absolute inset-0" style={{ backfaceVisibility: 'hidden' }}>
			  <Image src={ASSETS.ROLE_BACK} alt="Role Back" fill className="object-contain drop-shadow-[0_0_20px_rgba(0,0,0,0.4)]" />
			  {showClickText && (
				  <div className="absolute inset-0 flex items-center justify-center">
                <span className="bg-black/60 px-4 py-2 rounded-full border border-white/20 text-white text-[10px] sm:text-xs font-bold tracking-widest uppercase animate-pulse shadow-xl">
                  Cliquez
                </span>
				  </div>
			  )}
