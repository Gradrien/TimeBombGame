...
export function SettingsMenu() {
  ...
  const { gameState, playerId, socket, setReviewingCards, setReviewingRole } = useGameStore();
  ...
  const handleSurrenderVote = () => {
	if (gameState && socket) {
	  socket.emit('voteSurrender', gameState.roomId, playerId);
	}
  };
  ...
  return (
	  <div className="fixed top-4 right-4 z-50" ref={menuRef}>
		...
		{isOpen && (
			<div className="absolute z-[9999] right-0 mt-3 w-64 bg-[#1a1510]/95 backdrop-blur-md border border-[#8a6842]/50 rounded-2xl shadow-2xl overflow-hidden flex flex-col font-serif uppercase tracking-wide text-sm animate-in slide-in-from-top-2 fade-in duration-200">
			  ...
			  {gameState?.status === 'PLAYING' && (
				  <>
					<button onClick={() => { setReviewingRole(true); setIsOpen(false); }} className="px-5 py-4 text-left text-[#f3e7d3] hover:bg-[#8a6842]/30 hover:text-[#c9a56d] transition-colors border-b border-[#8a6842]/20">
					  Revoir mon rôle
					</button>
					<button onClick={() => { setReviewingCards(true); setIsOpen(false); }} className="px-5 py-4 text-left text-[#f3e7d3] hover:bg-[#8a6842]/30 hover:text-[#c9a56d] transition-colors border-b border-[#8a6842]/20">
					  Revoir mes câbles
					</button>
