...
const ROLE_INFO: Record<Role, RoleInfo> = {
  ...
  BROUILLEUR: {
    name: 'Brouilleur',
    team: 'Syndicat du Crime',
    desc: `Vous travaillez pour Moriarty. Votre présence brouille la Loupe (${Math.round(BROUILLEUR_JAM_CHANCE * 100)}% de chance) si on l'utilise sur vous.`,
    color: '#a855f7',
  },
};
...
export function RoleReveal({
  ...
  return (
      <motion.div
          ...
          className="fixed inset-0 z-[100] bg-ink-deep overflow-hidden flex items-center justify-center"
      >
        {/* Dark steampunk texture backdrop */}
        <div className="absolute inset-0 opacity-10 pointer-events-none"
             style={{backgroundImage: "url('/assets/textures/metal_plate.png')", backgroundSize: 'cover'}} />

        <AnimatePresence mode="wait">
          {!revealed ? (
              /* State 1: card back (hidden, or waiting for the others) */
              <motion.div
                  key="hidden"
              ...
              >
                ...
                <motion.p
                    ...
                    className="mt-8 text-cream font-serif tracking-[0.2em] uppercase text-xs sm:text-sm z-10 text-center px-4"
                >
                  {isConfirming ? 'En attente des autres joueurs...' : 'Touchez pour révéler votre rôle'}
                </motion.p>
              </motion.div>
          ) : (
              /* State 2: character reveal */
              <motion.div
                  key="revealed"
                  initial={{opacity: 0}}
                  animate={{opacity: 1}}
              ...
              >
                ...
                <div className="relative w-full h-[45vh] landscape:h-full landscape:w-1/2 flex items-end justify-center shrink-0">
                  ...
                  <div className="absolute bottom-0 left-0 w-full h-16 sm:h-24 landscape:h-32 bg-gradient-to-t from-ink-deep to-transparent pointer-events-none" />
                </div>

                {/* Right: copy and confirm action */}
                <motion.div
                ...
                <span className="text-xs sm:text-sm font-bold uppercase tracking-[0.2em] sm:tracking-[0.3em]" style={{color: config.color}}>
                  {config.team}
                    ...
                    <h1 className="text-3xl sm:text-5xl landscape:text-4xl lg:landscape:text-6xl font-serif font-black text-cream uppercase tracking-tighter">
                      {config.name}
                    </h1>
                  </div>
                  ...
                  <p className="text-cream/80 text-sm sm:text-lg landscape:text-base lg:landscape:text-xl leading-relaxed max-w-md font-serif italic">
                    "{config.desc}"
                  </p>
