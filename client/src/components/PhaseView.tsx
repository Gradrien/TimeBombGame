...
const RoleReveal = ({ role, revealed, isReady, setRevealed }: any) => {
  ...
  const [skinIndex] = useState(() => {
	if (role === 'SHERLOCK') return Math.floor(Math.random() * 5) + 1; // 1 à 5
	if (role === 'MORIARTY') return Math.floor(Math.random() * 3) + 1; // 1 à 3
	return 1;
  });

  return (
	  <div
		  className={`relative w-40 h-56 sm:w-60 sm:h-80 landscape:w-40 landscape:h-56 transition-transform duration-500 ${!revealed && !isReady ? 'cursor-pointer hover:scale-105' : 'cursor-default'}`}
...
export function PhaseView() {
  ...
  }, [gameState?.phase, gameState?.currentRound, socket?.id]);

  if (!gameState || !socket) return null;

  const me = gameState.players.find(p => p.id === socket.id);
  const isReady = gameState.readyPlayers.includes(socket.id || '') || isConfirming;
  ...
  return (
	  <div className="flex flex-col h-screen w-full bg-black/40 text-white overflow-hidden select-none">
		...
		{/* HEADER FIXE */}
		<div className="h-20 sm:h-28 landscape:h-16 flex items-center justify-center shrink-0 w-full px-4">
		...
		<div className="flex-1 w-full flex items-center justify-center">
		  {gameState.phase === 'ROLE_REVEAL' ? (
			  ...
			  /* LE SECRET DE L'ORBITE : Le conteneur tourne à 180° */
			  <div className={`flex flex-wrap justify-center gap-3 sm:gap-6 relative w-full max-w-4xl px-4 transition-all duration-700 ease-in-out ${isShuffling ? 'rotate-180' : 'rotate-0'}`}>
				{shuffledCards.map((type: string, i: number) => (
					/* LE SECRET DE L'ORBITE : Les cartes tournent à l'envers (-180°) pour rester droites ! */
		...
		<div className="h-32 landscape:h-24 shrink-0 flex flex-col items-center justify-center gap-4 sm:gap-6 w-full">
		  {!revealed && gameState.phase === 'CARD_REVEAL' && !isReady ? (
			  <button onClick={revealCardsOneByOne} className="bg-amber-500 text-black px-8 py-3 sm:px-12 sm:py-4 rounded-full font-bold text-xs sm:text-sm tracking-widest hover:bg-amber-400 transition-colors shadow-lg uppercase">
				VÉRIFIER MES CÂBLES
			  </button>
		  ) : revealed ? (
			  <button
			  ...
			  >
				{isConfirming ? 'EN ATTENTE DES AUTRES JOUEURS...' : "J'AI COMPRIS"}
			  </button>
		  ) : (
